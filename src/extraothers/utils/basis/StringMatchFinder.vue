package com.javaee.onlinehosbackend.extraother.utils.basis;


import com.javaee.onlinehosbackend.extraother.utils.coordinate.AbstractCoordinate;
import com.javaee.onlinehosbackend.extraother.utils.coordinate.IntCoordinate;
 ...
 **/
@Deprecated
public class StringMatchFinder implements IteratorAbstractCoordinatInteger>> {
     ...
     * 字符串的字符迭代器
     */
    private final IteratorInteger> charIterator;
     ...
     * 坐标点仅使用x,y
     */
//    private final ListCoordinateInteger>> foundIndex = new ArrayList<>();


    private final StringMatcher matcher;

    private AtomicReferenceAbstractCoordinateInteger>> next;

    /**
     * 当前的节点, 初始值为null
     */
    private AtomicReferenceCharMatchNode> node = new AtomicReference<>();

    /**
     * 是否下一个开始查询， 一般与head结合
     */
    private final PredicateCharacter> canFind;
     ...
     */
    @Override
    public AbstractCoordinateInteger> next() {
        // 是否是第一次执行此方法
        boolean first = false;
        // 如果匹配索引不是0, 则说明已经有一个值了，则此次为查询下一个值
        AbstractCoordinatenteger> forReturn;
        AbstractCoordinateInteger> forNext;
        first = index.get() == 0;
        if (!first) {
...
//        }
    ...
    public static StringMatchFinder of(IteratorInteger> find, String... words) {
        return new StringMatchFinder(find, words);
    }

    public static StringMatchFinder of(IteratorInteger> find, char head, String... words) {
        return new StringMatchFinder(find, head, words);
    }

    public static StringMatchFinder of(IteratorInteger> find, StringMatcher matcher) {
    ...
    public StringMatchFinder(IteratorInteger> find, String... words) {
    ...
    public StringMatchFinder(IteratorInteger> find, char head, String... words) {
     ...
     * @param matcher
     */
    public StringMatchFinder(IteratorInteger> find, StringMatcher matcher) {
