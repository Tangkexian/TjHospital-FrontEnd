      ...
      <div class="diagnostic">
        ...
        <div class="base-box">
          ...
          <div class="base-line">
            ...
            <div class="base-item">
              <div class="base-item-title">导员：</div>
...
export default {
  ...
  methods: {
    ...
    async openDrawer2() {
      ...
      //获得诊断历史
      const url4 =
        "http://localhost:8080/api/Registration/GetPatientInfoByDoctorID/" +
        this.doctorId;
      var requestOptions4 = {
        method: "GET",
        redirect: "follow",
      ...
      for (let i = 0; i < this.treatmentRecords.length; i++) {
        ...
        const url =
          "http://localhost:8080/api/Chatrecord/getChatRecord?RecordId=" +
    ...
    async initial() {
      ...
      //初始化今日挂号病人
      let api =
        "http://localhost:8080/api/Registration/commit?doctorId=" +
        this.doctorId;
      try {
      ...
      //初始化药品,获得药品名称和药品规格
      const url = "http://localhost:8080/api/Medicine/GetAllMedicinesInfo";
      try {
      ...
      //获得病假申请
      // const url3 =
      //   "http://localhost:8080/api/Leave/GetLeaveApplicationsByDoctor?doctorId=" +
      //   this.doctorId;
      // var requestOptions3 = {
      //   method: "GET",
      //   redirect: "follow",
    ...
    async getMessages(recordId) {
      ...
      // this.$Axios.get("http://localhost:8080/api/Chatrecord/getChatRecord?", {
      //   params: {
      //     RecordId: recordId,
      //   },
      // })
    ...
    async sendMessage() {
      ...
      fetch(
        "http://localhost:8080/api/Chatrecord/addChatRecord",
        requestOptions
      )
    ...
    async nextPatient() {
      ...
      const birthYear = parseInt(this.patients[i].birthDate[0]);
    ...
    enter() {
      ...
      fetch("http://localhost:8080/api/Confirm", requestOptions)
        .then((response) => response.text())
        .then((result) => console.log(result))
        .catch((error) => console.log("error", error));
    },
    ...
    async getHistory() {
      const url =
        "http://localhost:8080/api/DiagnosedHistory/GetDetailPre?patientId=" +
        this.patients[this.num - 1].patientId;
      //const testurl="http://localhost:8080/api/DiagnosedHistory/GetDetailPre?patientId=2151895"
      var requestOptions = {
        method: "GET",
        redirect: "follow",
      };
      ...
      const url2 =
        "http://localhost:8080/api/Leave/Offline?patientId=" +
        this.patientId +
        "&doctorId=" +
        this.doctorId +
        "&period=" +
    ...
    agree(id) {
      ...
      console.log(id);
      const url =
        "http://localhost:8080/api/Leave/ratify?leaveNoteId=" +
        id +
        "&status=1";
