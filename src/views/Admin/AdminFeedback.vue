...
export default {
  ...
  methods: {
    // 刷新表格
    getTable() {
      fetch("http://localhost:8080/api/Comment/GetAllFeedbacks", {
        method: 'GET',
        redirect: 'follow'
        ...
        .then(result => {
          ...
          for (let i = 0; i < this.tableItems.length; ++i) {
            ...
            // 获取医生姓名
            fetch("http://localhost:8080/api/Doctors/id?id=" + this.tableItems[i]["医生ID"], {
              method: 'GET',
              redirect: 'follow'
            }).then(response => response.text())
              .then(result => {
    ...
    // 删除反馈评价
    deleteItem() {
      fetch("http://localhost:8080/api/Comment/DeleteFeedback?diagnosedId="+this.tableItems[this.selectedRow]["诊断ID"], {
        method: 'DELETE',
        redirect: 'follow'
      }).then(response => response.text())
        .then(() => this.getTable())
