        ...
        <va-data-table :items="tableItems" :columns="tableColumns" :filter-method="filterFunction" :per-page="perPage"
          ...
          <template #headerAppend>
            <tr class="table-curd__slot">
              ...
              <th>
                <va-button style="width:99%; font-weight: 100;" color="backgroundElement" @click="addItem">
                  添加
                </va-button>
              </th>
            </tr>
...
export default {
  ...
  methods: {
    ...
    addItem() {
      ...
      for (; l < r; l++) {
        let raw = JSON.stringify({
          ...
          "dateTime": this.newDate + "T00:00:00",
        ...
        fetch("http://localhost:8080/api/Consultationinfo/AddConsult", requestOptions)
    ...
    deleteItem(rowIndex) {
      ...
      let raw = JSON.stringify({
        ...
        "dateTime": this.strToDate(this.tableItems[rowIndex]["日期"]),
      ...
      fetch("http://localhost:8080/api/Consultationinfo/CancelConsult", requestOptions)
        .then(response => response.text())
        .then(() => {
          this.getTable();
    ...
    confirmUpdate() {
      ...
      fetch("http://localhost:8080/api/Consultationinfo/ChangeConsult", requestOptions)
        .then(response => response.text())
        .then(
          this.getTable()
        )
    ...
    // 更新表格
    getTable() {
      fetch("http://localhost:8080/api/Consultationinfo/AllConsultInfo", {
        method: 'GET',
        redirect: 'follow'
      }).then(response => response.text())
        .then(result => {
          ...
          for (let i = 0; i < result.length; ++i) {
            this.tableItems.push({
              ...
              "日期": this.arrToDate(result[i].date),
    ...
    getInfo() {
      ...
      axios.get('http://localhost:8080/api/Consultationinfo/GetAllRooms')
        .then(response => {
          for (let i = 0; i < response.data.length; i++) {
            const item = response.data[i];
  ...
  watch: {
    ...
    newID(newVal) {
      ...
      fetch("http://localhost:8080/api/Doctors/id?id=" + newVal, {
        method: 'GET',
        redirect: 'follow'
      }).then(response => response.text())
        .then(result => {
