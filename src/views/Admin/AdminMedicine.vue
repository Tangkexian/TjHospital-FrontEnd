...
export default {
  ...
  methods: {
        ...
        .then(result => {
          ...
          for (let i = 0; i < result.length; ++i) {
            ...
            this.tableItems.push({ // 填入表项
              "药品名": result[i].id.medicineName,
              "生产单位": result[i].id.manufacturer,
              "生产日期": this.dateString(result[i].id.productionDate),
    ...
    // date 格式转换
    dateString(date) {
      return date[0].toString()+"-"+date[1].toString().padStart(2,'0')+"-"+date[2].toString().padStart(2,'0');
    },
    ...
    toRecord() {
      ...
      // 从后端获取清理记录
      fetch("http://localhost:8080/api/Medicine/GetCleanedMedicines", {
        method: 'GET',
        redirect: 'follow'
      }).then(response => response.text())
        .then(result => {
          ...
          for (let i = 0; i < result.length; ++i) {
            this.tableItems.push({
              ...
              "生产日期": this.dateString(result[i].productionDate),
              "清理日期": this.dateString(result[i].cleanDate),
    ...
    addItem() {
      ...
      fetch("http://localhost:8080/api/Medicine/AddStock", requestOptions)
        .then(response => response.text())
        .then(result => {
          if (result != "成功") {
            alert("参数错误！");
    ...
    // 删除项
    deleteItem(rowIndex) {
      fetch("http://localhost:8080/api/Medicine/CleanMedicine"
