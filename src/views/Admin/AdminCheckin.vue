...
export default defineComponent({
  ...
  methods: {
        ...
        .then((response) => { // 使用箭头函数
          ...
          for (let i = 0; i < response.data.length; ++i) { // 修改这里的response.data.length
            ...
            this.CheckinInfo.push({
              ...
              "appointmentDate": this.arrToDate(response.data[i].appointmentDate),
              "period": periodText,
    ...
    strToDate(str) {
      let arr = str.split("-");
      return arr[0] + "-" + arr[1].toString().padStart(2, '0') + "-" + arr[2].toString().padStart(2, '0') + "T00:00:00";
    },
    ...
    manual(index) {
      ...
      const data = {
        ...
        time: this.strToDate(this.CheckinInfo[index].appointmentDate),
        period: period,
      };
      console.log(data);
      // 发送请求
