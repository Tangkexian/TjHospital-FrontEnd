...
<template style="font-family: AliRegular;">
  <div class="parent-box">
    ...
    <div class="nmain">
      ...
      <div class="main-item">
        <va-date-picker stateful v-model="value" @click="fetchData(value)" class="ndate data" />
      </div>
    </div>
    ...
    <div class="button-groups nbtns" v-if="true">
      <div class="btn-line">
...
export default {
  ...
  methods: {
    ...
    fetchData(date) {
      ...
      this.formattedDate = `2024-${m}-${d}`
      // console.log(formattedDate);

      const formattedMonth = m.toString().padStart(2, '0')
      const formattedDay = d.toString().padStart(2, '0')
      this.formattedDate = `2024-${formattedMonth}-${formattedDay}`

      console.log(this.formattedDate);
      // 新增部分
      this.$Axios
        .get('/api/Registration/GetRegist?', {
          params: {
    ...
    //       // 在this.$Axios请求成功的回调函数内部调用Generate函数
    ...
    sendData(per) {
      ...
      this.Generate(dataString, (qrCodeSuccess) => {
        ...
        const url = '/api/Registration/regist'
        const data = {
          patientId: sessionStorage.getItem('userID'),
          doctorId: this.$route.params.selectedId,
          time: this.formattedDate + "T00:00:00",
          period: per + 1,
        ...
        this.$Axios
          .post(url, data)
          .then((response) => {
            // 在this.$Axios请求成功的回调函数内部
            console.log(response)
            self.alertText = '预约 ' + self.$route.params.selectedDoctor + ' 医生成功！<br>' + '时间：' + self.formattedDate + '<br>预约二维码：'
            self.showModal = true
